...
import { getRegistrations, getMyPassIds, saveMyPassId, findRegistration, formatDisplayPhone, getAgeCategoryDetails } from '../utils/storage';
import { EventPass } from './EventPass';
import { shareWhatsAppWithPassImage } from '../utils/whatsapp';
import { Search, PlusCircle, Ticket, User, CheckCircle2, QrCode, Phone, SearchCheck } from 'lucide-react';
...
export const AttendeePassesList: React.FC<AttendeePassesListProps> = ({
  ...
  const loadPasses = () => {
    ...
    // Filter to only passes registered on this device / retrieved by phone number
    const myPasses = all.filter(r => myIds.includes(r.ticketId));
